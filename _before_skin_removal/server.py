...
API:
  GET  /                     面板页面（静态托管项目目录）
  GET  /api/themes           列出已安装主题与当前激活项（含样式脱节标记）
  ...
  GET  /api/logs             操作日志
  GET  /api/template         参数 schema / 区域定义
  GET  /api/theme-params/<id>        读取主题参数
  GET  /api/theme-image/<id>         主题背景图
  GET  /api/export/<id>              导出主题包（zip 下载）
  POST /api/switch  {id}             切换主题
  POST /api/restore                  还原官方样式（含移除 CDP 皮肤）
  POST /api/remove  {id}             删除主题
  POST /api/install                  安装主题包（body 为 zip 二进制，?name=xxx.zip）
  POST /api/repair                   注入自检与修复（CDP 补注 + 旧模板迁移）
  POST /api/rebuild/<id>             用当前模板重生成样式（升级适配）
  ...
  POST /api/theme-params/<id> {params}  更新主题参数
"""
import argparse
import base64
...
import theme_engine as tengine
import tray
import updater
...
def _zip_theme_meta(zp):
    try:
        with zipfile.ZipFile(zp) as z:
            return json.loads(z.read('meta.json').decode('utf-8-sig'))
    except Exception:
        return None


def _upgrade_builtins(cfg):
    """已安装的内置主题若落后于包内 zip 版本，静默升级（最后恢复原激活主题，不打断使用）。"""
    seed_dir = os.path.join(ROOT, 'themes')
    if not os.path.isdir(seed_dir):
        return
    active_before = cfg.get('active')
    upgraded = []
    for name in sorted(os.listdir(seed_dir)):
        zp = os.path.join(seed_dir, name, name + '.zip')
        if not os.path.isfile(zp):
            continue
        meta = _zip_theme_meta(zp) or {}
        tid = meta.get('id') or name
        cur = (cfg.get('themes') or {}).get(tid)
        if not cur or not cur.get('builtin'):
            continue
        if _version_tuple(meta.get('version')) > _version_tuple(cur.get('version')):
            try:
                ts.install(zp)
                upgraded.append(tid)
            except SystemExit as e:
                print('[seed] 内置主题 %s 升级失败: %s' % (tid, e))
    if upgraded:
        c = ts.load_config()
        if active_before and c.get('active') != active_before and active_before in c.get('themes', {}):
            ts.apply_theme(c, active_before)   # 恢复用户原本激活的主题
        print('[seed] 已升级内置主题: %s' % ', '.join(upgraded))


def seed_builtin_themes():
    """内置主题：首次无主题时种子安装；之后每次启动静默升级落后的内置主题。"""
    try:
        cfg = ts.load_config()
        _upgrade_builtins(cfg)
        if cfg.get('themes'):
            return
        seed_dir = os.path.join(ROOT, 'themes')
        if not os.path.isdir(seed_dir):
        ...
        for name in sorted(os.listdir(seed_dir)):
            ...
            if os.path.isfile(zp):
                try:
                    ...
                    print('[seed] 内置主题 %s 安装失败: %s' % (name, e))
        if installed:
            cfg = ts.load_config()
            if cfg.get('active') is None and installed:
                ts.apply_theme(cfg, installed[0])
            print('[seed] 已安装内置主题: %s' % ', '.join(installed))
    except Exception as e:
        print('[seed] 内置主题种子异常: %s' % e)
...
def current_dom_fingerprint():
    ...
    return {'fingerprint': fp, 'landmarks': res.get('landmarks', {}),
            ...
            'template_version': tengine.TEMPLATE_VERSION, 'ts': time.time()}
...
def snapshots_status():
    ...
    try:
        ...
        cur_hv = ''
    cur_tv = tengine.TEMPLATE_VERSION
    reasons = []
    if man is None:
        ...
        if int(man.get('template_version', 0)) < cur_tv:
            reasons.append('主题模板已升级到 v{0}'.format(cur_tv))
    if missing:
...
_SOURCE_FILES = ('server.py', 'dsh-skin.py', 'dsh_env.py', 'theme_engine.py',
...
def repair_injection():
    """注入自检与修复（DSH 无文件通道，只做 CDP 补注 + 旧模板迁移）。"""
    fixed = []
    try:
        migrated = ts.migrate_all_themes()
    except Exception as e:
        migrated = []
        add_log('主题模板迁移失败: {0}'.format(e), 'error')
    if migrated:
        fixed.append('重建旧模板主题: ' + '、'.join(migrated))
    cfg = ts.load_config()
    active = cfg.get('active')
    if (active and active in cfg.get('themes', {})) or enhance_engine.load_state().get('enabled'):
        if not cdp_skin.WEBSOCKET_AVAILABLE:
...
def themes_payload():
    ...
    for tid, t in cfg['themes'].items():
        css = t.get('skin_css', '')
        themes.append({
            "id": tid, "name": t['name'], "version": t['version'],
            "appearance": t.get('appearance', 'light'), "active": cfg.get('active') == tid,
            "params": t.get('params', tengine.DEFAULT_PARAMS),
            "stale": tengine.theme_staleness(css),
            "has_image": os.path.isfile(os.path.join(ts.SKIN_ROOT, tid, 'background.png')),
            "builtin": bool(t.get('builtin', False)),
        })
    return api_ok({"active": cfg.get('active'), "themes": themes, "channel": _channel(cfg)})
...
def _token_usage_payload():
    """GET /api/token-usage 数据：缓存命中直接返回；否则同步重算（互斥）。
...
def _status_payload_uncached():
    ...
    # 无激活主题且增强关闭时视为健康（没有需要生效的内容，打标器本就不运行）。
    selectors = []
    enh_on = enhance_engine.load_state().get('enabled')
    if active in cfg.get('themes', {}) or enh_on:
        if cdp_skin.WEBSOCKET_AVAILABLE and cdp_skin.cdp_ready():
            res = cdp_skin.probe_regions()
            if res.get('ok'):
                counts = res.get('counts', {})
                for key, reg in tengine.resolve_regions().items():
    ...
    regions = tengine.diagnose_regions(cfg['themes'][active]['skin_css']) \
        if active in cfg.get('themes', {}) else []
    stale_themes = [tid for tid, t in cfg.get('themes', {}).items()
                    if tengine.css_needs_rebuild(t.get('skin_css'))]
    det = dsh_env.detect_report()
    return api_ok({
        "app": "dsh-skin",
        "version": ts.__version__,
        "product": det.get('product') or 'DeepSeek Harness',
        "template_version": tengine.TEMPLATE_VERSION,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _head_probe(self):
        ...
        if path == '/favicon.ico':
            status = 204
        elif path.startswith('/api/theme-image/'):
            tid = os.path.basename(path.rsplit('/', 1)[1])
            img = os.path.join(ts.SKIN_ROOT, tid, 'background.png')
            if re.match(r'^[a-z0-9_-]+$', tid) and os.path.isfile(img):
                ctype, length = 'image/png', os.path.getsize(img)
            else:
                status = 404
        else:
    ...
    def do_GET(self):
        ...
        if path == '/api/selectors':
            regions = tengine.resolve_regions()
            return self._json(api_ok({
                "overrides": tengine.load_overrides(),
                "file": tengine.OVERRIDES_FILE,
        ...
        if path == '/api/template':
            return self._json(api_ok({
                "schema": tengine.PARAM_SCHEMA,
                "defaults": tengine.DEFAULT_PARAMS,
                "templates": {"light": tengine.INJECT_TEMPLATE, "dark": tengine.DARK_INJECT_TEMPLATE},
                "regions": [{"key": k, "label": v["label"], "reliable": v.get("reliable", False),
                             "note": v.get("note", "")}
                            for k, v in tengine.resolve_regions().items()],
            }))
        if path.startswith('/api/theme-params/'):
            tid = os.path.basename(path.rsplit('/', 1)[1])
            cfg = ts.load_config()
            if tid not in cfg.get('themes', {}):
                return self._json(api_err('未安装主题 {0}'.format(tid)), 404)
            t = cfg['themes'][tid]
            return self._json(api_ok({"id": tid,
                                      "params": t.get('params', tengine.DEFAULT_PARAMS),
                                      "regions": tengine.diagnose_regions(t['skin_css'])}))
        if path.startswith('/api/preview-css/'):
            # 预览与真实 CDP 注入同源：用主题自身参数渲染最终 CSS（背景图内嵌 data URI），
            # 不再依赖前端占位替换。both/light 出双轨 CSS，快照靠 body[data-ds-dark-theme] 切暗态。
            tid = os.path.basename(path.rsplit('/', 1)[1])
            if not re.match(r'^[a-z0-9_-]+$', tid):
                return self._json(api_err('非法主题 id'), 404)
            cfg = ts.load_config()
            if tid not in cfg.get('themes', {}):
                return self._json(api_err('未安装主题 {0}'.format(tid)), 404)
            t = cfg['themes'][tid]
            app0 = t.get('appearance', 'light')
            img = os.path.join(ts.SKIN_ROOT, tid, 'background.png')
            if not os.path.isfile(img):
                img = ''
            gen_app = 'dark' if app0 == 'dark' else 'light'
            try:
                css = tengine.generate_inject_css(
                    t.get('params', tengine.DEFAULT_PARAMS), img, gen_app)
            except Exception as e:
                return self._json(api_err('预览 CSS 生成失败: {0}'.format(e)))
            return self._json(api_ok({'id': tid, 'css': css, 'appearance': app0,
                                      'template_version': tengine.TEMPLATE_VERSION}))
        if path.startswith('/api/theme-image/'):
            ...
            img = os.path.join(ts.SKIN_ROOT, tid, 'background.png')
            if not os.path.isfile(img):
                return self._json(api_err('主题无预览图'), 404)
            with open(img, 'rb') as f:
                return self._bin(f.read(), 'image/png')
        if path.startswith('/api/export/'):
            ...
            img = os.path.join(ts.SKIN_ROOT, tid, 'background.png')
            if not os.path.isfile(img):
                return self._json(api_err('主题「{0}」缺少 background.png，无法导出'.format(t.get('name', tid))), 404)
            meta = {"id": tid, "name": t.get('name', tid), "version": t.get('version', '1.0.0'),
                    "appearance": t.get('appearance', 'light'), "image": "background.png"}
            if t.get('params'):
            ...
            with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as z:
                ...
                z.writestr('skin.css', t['skin_css'].replace(t['image'], '{{IMAGE}}'))
            fname = re.sub(r'[^\w\u4e00-\u9fff-]+', '_', t.get('name', tid)) + '-theme.zip'
            add_log('已导出主题包「{0}」'.format(t.get('name', tid)), 'success')
            return self._bin(buf.getvalue(), 'application/zip', fname)
        if path == '/favicon.ico':
            self.send_response(204)
            self._cors()
            self.end_headers()
    ...
    def do_POST(self):
        ...
        if path == '/api/switch':
            ...
            tid = body.get('id')
            if not tid:
                return self._json(api_err('缺少 id'))
            res = run_ts(ts.switch, tid)
            if res['ok']:
                add_log('已切换主题「{0}」'.format(tid), 'success')
                self._try_cdp(log_prefix='切换后')
                res['data'] = {"id": tid, **self._live_state()}
            return self._json(res)

        if path == '/api/restore':
            res = run_ts(ts.restore)
            if res['ok']:
                add_log('已还原官方样式', 'success')
            return self._json(res)

        if path.startswith('/api/rebuild/'):
            tid = os.path.basename(path.rsplit('/', 1)[1])
            res = run_ts(ts.rebuild, tid)
            if res['ok']:
                add_log('已按当前模板重建主题「{0}」'.format(tid), 'success')
                self._try_cdp(log_prefix='重建后')
            return self._json(res)
        ...
        if path == '/api/probe':
            ...
            applied = None
            if body.get('apply'):
                applied = tengine.save_overrides(res.get('suggested', {}))
            ...
            if body.get('rebuild'):
                ...
                for tid, t in cfg.get('themes', {}).items():
                    if tengine.css_needs_rebuild(t.get('skin_css', '')):
                        ...
                        rebuilt.append(tid)
            _drift_cache['v'] = None
            add_log('已更新界面结构基线（%d 个区域，DSH %s）%s'
                    % (len(cur['fingerprint']), cur.get('dsh_version') or '?',
                       '；已重建主题: ' + ', '.join(rebuilt) if rebuilt else ''), 'success')
            invalidate_status()
            return self._json(api_ok({"baseline": cur, "rebuilt": rebuilt}))

        if path == '/api/selectors':
            body = self._body_json()
            if body.get('reset'):
                tengine.save_overrides({})
                add_log('已清空选择器覆盖', 'info')
                return self._json(api_ok({"overrides": {}}))
            regions = body.get('regions')
            if not isinstance(regions, dict):
                return self._json(api_err('缺少 regions'))
            saved = tengine.save_overrides(regions)
        ...
        if path == '/api/remove':
            tid = self._body_json().get('id')
            if not tid:
                return self._json(api_err('缺少 id'))
            res = run_ts(ts.remove, tid)
            if res['ok']:
                add_log('已删除主题「{0}」'.format(tid), 'success')
            return self._json(res)

        if path == '/api/install':
            data = self._read_body()
            if not data:
                return self._json(api_err('未收到主题包数据'))
            name = (qs.get('name') or ['theme.zip'])[0]
            if not name.lower().endswith('.zip'):
                return self._json(api_err('仅支持 .zip 主题包'))
            if data[:2] != b'PK':
            ...
            try:
                tmp.write(data)
                tmp.close()
                res = run_ts(ts.install, tmp.name)
                if res['ok']:
                    add_log('已安装并启用主题包 {0}'.format(name), 'success')
                    self._try_cdp(log_prefix='安装后')
                return self._json(res)
            finally:
        ...
        if path.startswith('/api/theme-params/'):
            tid = os.path.basename(path.rsplit('/', 1)[1])
            params = self._body_json().get('params')
            if not isinstance(params, dict):
                return self._json(api_err('缺少 params'))
            # 部分更新语义：先把已存参数铺底，再叠传入键——
            # norm_params 缺省填 DEFAULT，不铺底会把未提及的参数重置丢失
            existing = ts.load_config().get('themes', {}).get(tid, {}).get('params', {})
            merged = dict(existing)
            merged.update(params)
            res = run_ts(ts.update_params, tid, merged)
            if res['ok']:
                cfg = ts.load_config()
                t = cfg['themes'].get(tid, {})
                add_log('已更新「{0}」主题参数 (v{1})'.format(t.get("name", tid), t.get("version", "?")), 'success')
                self._try_cdp(log_prefix='调参后')
                res['data'] = {"id": tid, **self._live_state()}
            return self._json(res)
    ...
    def _try_cdp(self, log_prefix=''):
        ...
        try:
            ...
            if not r.get('ok') and r.get('err') not in ('running-without-cdp', 'stopped', 'no-active-theme'):
