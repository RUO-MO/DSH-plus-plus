...
json.dump({'capturedAt': 1, 'harness_version': '2.0.0',
           'template_version': T.TEMPLATE_VERSION, 'files': {}},
...
json.dump({'capturedAt': 1, 'harness_version': '1.5.0',
           'template_version': T.TEMPLATE_VERSION}, open(SV.SNAP_MANIFEST, 'w'))
st = SV.snapshots_status()
...
# 4) 模板升级 → stale + 缺文件
json.dump({'capturedAt': 1, 'harness_version': '2.0.0',
           'template_version': T.TEMPLATE_VERSION - 1}, open(SV.SNAP_MANIFEST, 'w'))
os.remove(os.path.join(tmp, SV.SNAP_FILES[0]))
st = SV.snapshots_status()
assert st['stale'] and st['missing'] and any('模板' in r for r in st['reasons'])
print('4) 模板升级+缺文件判 stale: PASS')
